...
# Renomeando para nome o Country Name
base_covid_2 <- PIB2019 %>% rename(nome="Country Name") %>%
# Dando Right join pela variável nome
  right_join(base_covid, by = "nome") %>% 
# Selecionar todas as variáveis resultantes menos o COuntry Code
  select(everything(), -`Country Code`) %>% 
# Renomear o Income Group da base covid para grupo_renda
  rename(grupo_renda="Income group") %>% 
# Mudando as classificações do grupo_renda
...
# Agrupando a base por grupo_renda
base_renda <- base_covid_2 %>% group_by(grupo_renda) %>% 
# Sumarizando informações por média, desvio padrão
  summarise(média=mean(casos_relativo, na.rm = T),
            desvio_padrão=sd(casos_relativo, na.rm = T),
            obs.=n()) %>%
  ungroup () %>% droplevels(.)
